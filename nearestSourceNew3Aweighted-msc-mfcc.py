...
def getAudioFranes(blockLength, indata):
    finished = False
    sample_index = 0
    data_blocks = []
    volumes = []
    ...
    while not finished:
        ...
        #data_block = signal.correlate(data_block, data_block)
        s,p = librosa.magphase(librosa.stft(data_block, n_fft=blockLength, hop_length=2*blockLength))
        aweighted = librosa.perceptual_weighting(s[1:],freqs[1:])
        #print(freqs)
        #print(aweighted)
        mean_block = np.mean(10**(aweighted/10))
        #print(mean_block)
        if (mean_block) > 0.001:
           #print((mean_block))
            volumes.append(mean_block)
...
def getAudioMfccValues(indata):
    for i in range(0, len(indata)):
        mfcc = librosa.feature.mfcc(indata[i],SAMPLE_RATE, n_mfcc=40, n_fft=blockLength, hop_length=blockLength)   #Computing MFCC values
        mfcc = round(np.max(mfcc),1)
    return mfcc       
...
def MSCMeanForAllFrames(indata, blockLength, fs):
    ...
    while not finished:
        ...
        dataBlockR = indata[block_length+sample_index :,4]
...
for root, dirs, files in os.walk(directory):
          ...
          for i in range (0, toIter):
              ...
              for file in files:
                  ...
                  if (str.find('speech') != -1 ):
                      ...
                      if len(labelsGlobal) > 0 and isStringMatched(index, str,labelsGlobal ):
                          ...
                          librosa_data, librosa_samplerate = librosa.load(str, sr = SAMPLE_RATE)
                          audioFrames = getAudioFranes(blockLength, librosa_data) 
                          mfccCurrent = getAudioMfccValues(audioFrames)
                          print(mscMeanCurrent)
                          print(mfccCurrent)
                          if file_index == 0:
                             coherence1.append(mscMeanCurrent)
                             mfcc1.append(mfccCurrent)
                          else:
                             coherence2.append(mscMeanCurrent)
                             mfcc2.append(mfccCurrent)
                      
                          if mscMeanCurrent + mfccCurrent > mscPrev + mfccPrev:
                             mscPrev = mscMeanCurrent
                             mfccPrev = mfccCurrent                                                         
              ...
              if len(labels) > 0:
                 ...
                 dataset["mfcc1"].append(mfcc1[0])
                 dataset["mfcc2"].append(mfcc2[0])                 
                 labelsGlobal.append(labels[0])
                 labelsGlobal.append(labels[1])
     
print(dataset)

with open ('C:/Users/Vijaya/PhD/Audio_DataSetmsc_mfcc_3d_without_smooth_0.6.pickle', 'wb' ) as f:
