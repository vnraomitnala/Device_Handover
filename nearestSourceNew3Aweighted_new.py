...
dataset = {"speaker": [], "microphone": []  }

directory = os.fsencode('C:\\Users\\Vijaya\\PhD\\MicrophoneArray_Dataset\\speakerSet\\')
...
def isStringMatched (index, fileStr, indata):
    ...
    for i in range(0, len(indata),2):
        ...
        length = fileStr.find("speech")
        microphoneValue = str[length-3 : length-1]
        
        if (indata[i+1] == microphoneValue):        
            found = True
...
for root, dirs, files in os.walk(directory):
          ...
          for i in range (0, toIter):
                      ...
                      if len(labelsGlobal) > 0 and isStringMatched(index, str,labelsGlobal ):
                          ...
                          if mscMeanCurrent > mscPrev:
                             ...
                             if len(labels) > 0:
                                 ...
                                 microphoneValue = str[length-3 : length-1]
                                 shuffle(labels, labels[0],  (str[len(str)-6 : len(str)-4]), labels[1], microphoneValue)
                             else:
              ...
              index = index + 1  
              if len(labels) > 0:
                 dataset["speaker"].append(labels[0]) 
                 dataset["microphone"].append(labels[1])                 
...
with open ('C:/Users/Vijaya/PhD/Audio_DataSetmsc_tmptmp.pickle', 'wb' ) as f:
    pickle.dump(tmp, f)  
